...
TRUNCATE TABLE ST_IMDB_BASE_TITLE_RATINGS;

INSERT INTO ST_IMDB_BASE_TITLE_RATINGS (
	TCONST_NK
	,AVERAGERATING
	,NUMVOTES
	,SRC_ID
...
SELECT TCONST_NK AS TCONST_NK
	...
	,NOTE AS NOTE
FROM SV_IMDB_BASE_TITLE_RATINGS S
ORDER BY TCONST_NK;

ANALYZE ST_IMDB_BASE_TITLE_RATINGS;
...
FROM (
	...
	FROM ST_IMDB_BASE_TITLE_RATINGS
	GROUP BY TCONST_NK
		,SRC_ID
	) AS S
GROUP BY S.SRC_ID;
...
SELECT 'IMDB_BASE'
	...
	,CASE
		...
		ELSE 1
		END
	,'DATA SOURCE IS EMPTY. (TABLE ST_IMDB_BASE_TITLE_RATINGS).'
	,CASE
		WHEN SUM(ROW_COUNT) = SUM(DISTINCT_PK_COUNT)
			THEN 0
		ELSE 1
		END
	,'DATA SOURCE FAILED PRIMARY KEY TEST. (TABLE ST_IMDB_BASE_TITLE_RATINGS).'
